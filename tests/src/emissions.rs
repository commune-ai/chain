...
use pallet_chain::{Active, Consensus, Founder, PruningScores, Rank, Trust, ValidatorTrust};
...
use pallet_chain::{
    ...
    MaxWeightAge, MinValidatorStake, MinimumAllowedStake, Pallet as ChainMod, RegistrationBlock,
...
fn test_pruning() {
    new_test_ext().execute_with(|| {
        ...
        let lowest_priority_uid: u16 = ChainMod::<Test>::get_lowest_uid(netuid, false).unwrap();
        assert_eq!(lowest_priority_uid, prune_uid);

        // Register new module
        let new_key = n as u32 + 1;
        let prune_key = ChainMod::<Test>::get_key_for_uid(netuid, prune_uid).unwrap();
        assert_ok!(register_module(netuid, new_key, stake_per_module, false));

        // Assert new module is registered
        let is_registered: bool = ChainMod::<Test>::key_registered(netuid, &new_key);
        ...
        let is_prune_registered: bool = ChainMod::<Test>::key_registered(netuid, &prune_key);
        ...
        assert_eq!(
            ChainMod::<Test>::get_emission_for(new_netuid)
                .get(0)
                .copied()
                .unwrap_or_default(),
            0
        ...
        assert_eq!(
            ChainMod::<Test>::get_lowest_uid(new_netuid, false),
            Some(1)
        );
    });
}
...
fn test_lowest_priority_mechanism() {
    new_test_ext().execute_with(|| {
        ...
        let lowest_priority_uid: u16 =
            ChainMod::<Test>::get_lowest_uid(netuid, false).unwrap_or(0);
...
fn calculates_blocks_until_epoch() {
    new_test_ext().execute_with(|| {
        let blocks_until_next_epoch = |netuid, tempo, block_number| {
            Tempo::<Test>::set(netuid, tempo);
            ChainMod::<Test>::blocks_until_next_epoch(netuid, block_number)
        };
...
fn test_incentives() {
    new_test_ext().execute_with(|| {
        ...
        SubnetConsensusType::<Test>::insert(netuid, SubnetConsensus::Linear);
        let mut params = ChainMod::<Test>::subnet_params(netuid);
...
fn test_founder_share() {
    new_test_ext().execute_with(|| {
        ...
        let subnet_params = ChainMod::<Test>::subnet_params(netuid);
        ...
        let founder_balance = ChainMod::<Test>::get_balance(&founder_key);
        ...
        let treasury_balance = ChainMod::<Test>::get_balance(&treasury_address);
        assert!(
...
mod utils {
    use crate::mock::Test;
    use pallet_chain::{Consensus, Dividends, Emission, Incentive, Rank, Trust};
...
fn test_1_graph() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ChainMod::<Test>::get_delegated_stake(&key),
...
fn test_10_graph() {
    ...
    new_test_ext().execute_with(|| {
        ...
        for i in 0..n as u16 {
            assert_eq!(
                from_nano(ChainMod::<Test>::get_delegated_stake(&(i as u32))),
...
fn yuma_weights_older_than_max_age_are_discarded() {
    new_test_ext().execute_with(|| {
        ...
        let miner_uid = ChainMod::<Test>::get_uid_for_key(yuma_netuid, &yuma_miner_key).unwrap();
        let validator_uid =
            ChainMod::<Test>::get_uid_for_key(yuma_netuid, &yuma_validator_key).unwrap();
        ...
        let miner_incentive = ChainMod::<Test>::get_incentive_for(yuma_netuid)
            .get(miner_uid as usize)
            .copied()
            .unwrap_or_default();
        let miner_dividends = ChainMod::<Test>::get_dividends_for(yuma_netuid)
            .get(miner_uid as usize)
            .copied()
            .unwrap_or_default();
        let validator_incentive = ChainMod::<Test>::get_incentive_for(yuma_netuid)
            .get(validator_uid as usize)
            .copied()
            .unwrap_or_default();
        let validator_dividends = ChainMod::<Test>::get_dividends_for(yuma_netuid)
            .get(validator_uid as usize)
            .copied()
            .unwrap_or_default();
...
fn test_emission_exploit() {
    new_test_ext().execute_with(|| {
        ...
        let stake_accumulated = ChainMod::<Test>::get_delegated_stake(&yuma_badactor_key);
        // User will now unstake and register another subnet.
        assert_ok!(ChainMod::<Test>::do_remove_stake(
        ...
        let _ = ChainMod::<Test>::register_subnet(origin.clone(), network.clone(), None);
        assert_ok!(ChainMod::<Test>::register(
            origin.clone(),
            network,
            name,
            address,
        ...
        assert_ok!(ChainMod::<Test>::add_stake(
            origin,
            yuma_badactor_key,
        ...
        let stake_accumulated_two = ChainMod::<Test>::get_delegated_stake(&yuma_badactor_key);
        assert_ok!(ChainMod::<Test>::do_remove_stake(
            get_origin(yuma_badactor_key),
            yuma_badactor_key,
        ...
        let badactor_balance_after = ChainMod::<Test>::get_balance(&yuma_badactor_key);
        ...
        let honest_stake = ChainMod::<Test>::get_delegated_stake(&honest_actor_key);
...
fn test_tempo_compound() {
    new_test_ext().execute_with(|| {
        ...
        let fast = ChainMod::<Test>::get_delegated_stake(&f_key);
        let slow = ChainMod::<Test>::get_delegated_stake(&s_key);
...
fn test_subnet_deregistration_based_on_emission() {
    new_test_ext().execute_with(|| {
        ...
        let _ = ChainMod::<Test>::do_register_subnet(get_origin(3), universal_vec.clone(), None);
        assert_ok!(ChainMod::<Test>::do_register(
            get_origin(3),
            universal_vec.clone(),
            universal_vec.clone(),
            universal_vec.clone(),
...
fn weight_setting_delegation() {
    new_test_ext().execute_with(|| {
        ...
        pallet_chain::Pallet::<Test>::update_module(
        ...
        pallet_chain::WeightSettingDelegation::<Test>::set(NETUID, child_key, Some(parent_key));
