...
fn subnet_update_changes_all_parameter_values() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            pallet_chain::MinValidatorStake::<Test>::get(netuid),
            min_validator_stake
        );
        assert_eq!(
        ...
        assert_eq!(ChainMod::get_total_subnets(), 1);
...
fn removes_subnet_from_storage() {
    new_test_ext().execute_with(|| {
        ...
        params!(not_exists);
        assert_eq!(ChainMod::get_total_subnets(), 0);
...
fn update_subnet_verifies_names_uniquiness_integrity() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(update_params(0, 0, ChainMod::subnet_params(0)));
        assert_err!(
            update_params(0, 0, ChainMod::subnet_params(1)),
...
fn subnet_is_replaced_on_reaching_max_allowed_modules() {
    new_test_ext().execute_with(|| {
        ...
        let subnet_amount = ChainMod::get_total_subnets();
        ...
        let subnet_amount = ChainMod::get_total_subnets();
        let total_module_amount = ChainMod::global_n_modules();
        assert_eq!(subnet_amount, expected_subnet_amount);
