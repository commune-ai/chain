...
fn module_is_registered_correctly() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            ChainMod::do_register(get_origin(0), network, name, address, 0, None),
...
fn registers_a_module_and_storage_values_correctly() {
    new_test_ext().execute_with(|| {
        ...
        let module_uid = ChainMod::get_uid_for_key(netuid, &key).unwrap();
        assert_eq!(ChainMod::get_uid_for_key(netuid, &key).unwrap(), 0);
...
fn registers_module_delegating_stake() {
    new_test_ext().execute_with(|| {
        ...
        ChainMod::add_balance_to_account(&key, stake_amount * n as u64);

        for module_key in 0..n {
            delegate_register_module(netuid, key, module_key, stake_amount)
                .expect("delegate register module failed");
            let stake_to_module = ChainMod::get_stake_to_module(&key, &module_key);
...
fn deregister_within_subnet_when_limit_is_reached() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(ChainMod::get_delegated_stake(&0), to_nano(10_000));
        assert_eq!(ChainMod::get_delegated_stake(&1), to_nano(5_000));
        ...
        assert_eq!(ChainMod::get_delegated_stake(&2), to_nano(15_000));
        assert_eq!(ChainMod::get_delegated_stake(&1), to_nano(5_000));
...
fn deregister_subnet_with_dangling_keys() {
    new_test_ext().execute_with(|| {
        ...
        let netuid = ChainMod::get_netuid_for_name("test1".as_bytes()).unwrap();
        ChainMod::remove_subnet(netuid);

        assert_eq!(StakeFrom::<Test>::get(key_b, key_b), 0);
        assert_eq!(ChainMod::get_balance(&key_b), stake_b + 1)
    });
}
...
mod module_validation {
    ...
    fn register_custom(netuid: u16, key: u32, name: &[u8], addr: &[u8]) -> DispatchResult {
        ...
        let is_new_subnet: bool = !ChainMod::if_subnet_exist(netuid);
        if is_new_subnet {
        ...
        ChainMod::register(origin, network, name.to_vec(), addr.to_vec(), key, None)
    }
    ...
    fn validates_module_on_update() {
        new_test_ext().execute_with(|| {
            ...
            let uid = ChainMod::get_uid_for_key(subnet, &key_1).unwrap();
            let params = ChainMod::module_params(0, &key_1, uid);
            ...
            assert_ok!(ChainMod::update_module(
                origin_1.clone(),
                subnet,
                b"test3".to_vec(),
                b"0.0.0.0:3".to_vec(),
    ...
    fn subnet_registration_validates_subnet_names() {
        new_test_ext().execute_with(|| {
            ...
            let register_subnet = |key, name: Vec<u8>| {
                ...
                ChainMod::register_subnet(get_origin(key), name.clone(), None)?;
                ChainMod::register(
                    get_origin(key),
                    name,
                    module_name.clone(),
...
fn new_subnet_reutilized_removed_netuid_if_total_is_bigger_than_removed() {
    new_test_ext().execute_with(|| {
        ...
        ChainMod::add_balance_to_account(&0, SubnetBurn::<Test>::get() + 1 + to_nano(10));
        let _ = ChainMod::register_subnet(get_origin(0), b"test".to_vec(), None);
        ChainMod::register(
            get_origin(0),
            b"test".to_vec(),
            b"test".to_vec(),
...
fn new_subnets_on_removed_uids_register_modules_to_the_correct_netuids() {
    ...
    fn register_module(netuid: u16, key: AccountId, stake: u64) {
        ...
        ChainMod::add_balance_to_account(&key, stake + SubnetBurn::<Test>::get() + 1);
        let _ = ChainMod::register_subnet(origin.clone(), network.clone(), None);
        ChainMod::register(origin, network.clone(), name, address, key, None).unwrap();

        let netuid = ChainMod::get_netuid_for_name(&network).unwrap();
        let uid = pallet_chain::Uids::<Test>::get(netuid, key).unwrap();
    ...
    new_test_ext().execute_with(|| {
        ...
        let add_emission = |netuid, id, emission| {
            let netuid =
                ChainMod::get_netuid_for_name(format!("test{netuid}").as_bytes()).unwrap();
...
fn test_subnet_immunity() {
    new_test_ext().execute_with(|| {
        ...
        ChainMod::add_balance_to_account(&0, SubnetBurn::<Test>::get());
        let _ = ChainMod::register_subnet(get_origin(0), b"net1".to_vec(), None);
        assert_ok!(ChainMod::register(
            get_origin(0),
            b"net1".to_vec(),
            b"mod1".to_vec(),
            b"127.0.0.1".to_vec(),
        ...
        ChainMod::increase_stake(&0, &0, 100000000000);

        ChainMod::add_balance_to_account(&1, 100000000001 + SubnetBurn::<Test>::get());

        assert_err!(
            ChainMod::register_subnet(get_origin(1), b"net2".to_vec(), None),
