...
fn test_ownership_ratio() {
    new_test_ext().execute_with(|| {
        ...
        let keys = ChainMod::get_keys(netuid);
        let voter_key = keys[0];
        let miner_keys = keys[1..].to_vec();
        let miner_uids: Vec<u16> =
            miner_keys.iter().map(|k| ChainMod::get_uid_for_key(netuid, k)).collect();
        ...
        let pre_delegate_stake_from_vector = ChainMod::get_stake_from_vector(&voter_key);
        ...
        for (i, d) in delegate_keys.iter().enumerate() {
            assert_ok!(ChainMod::add_stake(
                get_origin(*d),
                voter_key,
                stake_per_module,
            ));
            let stake_from_vector = ChainMod::get_stake_from_vector(&voter_key);
            assert_eq!(
                stake_from_vector.len(),
                pre_delegate_stake_from_vector.len() + i + 1
            );
        }
        let ownership_ratios = ChainMod::get_ownership_ratios(netuid, &voter_key);
        assert_eq!(ownership_ratios.len(), delegate_keys.len() + 1);

        let total_balance = keys
            .iter()
            .map(ChainMod::get_balance)
            .collect::<Vec<u64>>()
            .iter()
            .sum::<u64>();
        let total_stake = keys
            .iter()
            .map(|k| ChainMod::get_stake_to_module(k, k))
            .collect::<Vec<u64>>()
            .iter()
            .sum::<u64>();
        let total_delegate_stake = delegate_keys
            .iter()
            .map(|k| ChainMod::get_stake_to_module(k, &voter_key))
            .collect::<Vec<u64>>()
            .iter()
            .sum::<u64>();
        ...
        let result = ChainMod::set_weights(
...
fn test_ownership_ratio() {
    new_test_ext().execute_with(|| {
        ...
        for k in &keys {
            ...
            let pre_delegate_stake_from_vector = ChainMod::get_stake_from_vector(netuid, k);
            assert_eq!(pre_delegate_stake_from_vector.len(), 1); // +1 for the module itself, +1 for the delegate key on
            ...
            for (i, d) in delegate_keys.iter().enumerate() {
                ...
                let stake_from_vector = ChainMod::get_stake_from_vector(netuid, k);
                assert_eq!(
                    stake_from_vector.len(),
                    pre_delegate_stake_from_vector.len() + i + 1
                );
            ...
            let ownership_ratios: Vec<(U256, I64F64)> =
                ChainMod::get_ownership_ratios(netuid, k);
            ...
            let stake_from_vector = ChainMod::get_stake_from_vector(netuid, k);
            let stake: u64 = ChainMod::get_stake(netuid, k);
            let sumed_stake: u64 = stake_from_vector.iter().fold(0, |acc, (_a, x)| acc + x);
            let total_stake: u64 = ChainMod::get_total_subnet_stake(netuid);
...
fn test_emission_ratio() {
    new_test_ext().execute_with(|| {
        ...
        for i in 0..netuids.len() {
            ...
            let emission: u64 = ChainMod::calculate_network_emission(netuid, threshold);
            emissions_per_subnet.push(emission);
            let _expected_emission_factor: f64 = 1.0 / (netuids.len() as f64);
            let emission_per_block = ChainMod::get_total_emission_per_block();
...
fn test_emission_distribution_novote() {
    ...
    new_test_ext().execute_with(|| {
        ...
        let general_netuid_stake = from_nano(ChainMod::get_total_subnet_stake(netuid_general));
        let yuma_netuid_stake = from_nano(ChainMod::get_total_subnet_stake(netuid_yuma));
        let below_threshold_netuid_stake =
            from_nano(ChainMod::get_total_subnet_stake(netuid_below_threshold));
...
fn test_yuma_self_vote() {
    new_test_ext().execute_with(|| {
        ...
        set_weights(
            netuid_yuma,
            validator_key,
            [ChainMod::get_uid_for_key(netuid_yuma, &miner_key)].to_vec(),
            [1].to_vec(),
        );
        set_weights(
            netuid_yuma,
            validator_self_key,
            [ChainMod::get_uid_for_key(netuid_yuma, &miner_self_key)].to_vec(),
            [1].to_vec(),
        );
        assert_ok!(register_module(
            netuid_below_threshold,
        ...
        let stake_validator = ChainMod::get_stake(netuid_yuma, &validator_key);
        let stake_miner = ChainMod::get_stake(netuid_yuma, &miner_key);
        let stake_validator_self_vote = ChainMod::get_stake(netuid_yuma, &validator_self_key);
        let stake_miner_self_vote = ChainMod::get_stake(netuid_yuma, &miner_self_key);
        ...
        let general_netuid_stake = ChainMod::get_total_subnet_stake(netuid_general);
        let yuma_netuid_stake = ChainMod::get_total_subnet_stake(netuid_yuma);
        let below_threshold_netuid_stake =
            ChainMod::get_total_subnet_stake(netuid_below_threshold);
...
fn test_emission_activation() {
    new_test_ext().execute_with(|| {
        ...
        for (i, (name, initial_stake, should_activate)) in subnet_stakes.iter().enumerate() {
            let current_stake = ChainMod::get_total_subnet_stake(i as u16);
            if *should_activate {
                assert!(
...
fn test_active_stake() {
    new_test_ext().execute_with(|| {
        ...
        assert!(ChainMod::is_registered(9, &U256::from(10)));
        ...
        step_block(1);
        assert!(ChainMod::is_registered(9, &U256::from(12)));
        ...
        let uid_zero_dividends = ChainMod::get_dividends_for(9, 0);
        let uid_two_dividends = ChainMod::get_dividends_for(9, 2);
...
fn test_weight_age() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(ChainMod::set_weights(
        ...
        let passive_stake_before = ChainMod::get_total_stake_to(&U256::from(PASSIVE_VOTER));
        let active_stake_before = ChainMod::get_total_stake_to(&U256::from(ACTIVE_VOTER));

        step_block((TEMPO as u16) * 2);

        let passive_stake_after = ChainMod::get_total_stake_to(&U256::from(PASSIVE_VOTER));
        let active_stake_after = ChainMod::get_total_stake_to(&U256::from(ACTIVE_VOTER));
        ...
        let passive_stake_after_v2 = ChainMod::get_total_stake_to(&U256::from(PASSIVE_VOTER));
        let active_stake_after_v2 = ChainMod::get_total_stake_to(&U256::from(ACTIVE_VOTER));
