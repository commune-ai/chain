...
use pallet_chain::{
    Event as ChainEvent, GlobalParams, Pallet as PalletChain, SubnetParams, TotalStake,
};
...
impl<T: Config> Proposal<T> {
    ...
    fn execute_proposal(self) -> DispatchResult {
        PalletChain::<T>::add_balance_to_account(
            &self.proposer,
            PalletChain::<T>::u64_to_balance(self.proposal_cost).unwrap(),
        );
        ...
        match self.data {
            ...
            ProposalData::GlobalParams(params) => {
                PalletChain::<T>::set_global_params(params.clone())?;
                PalletChain::<T>::deposit_event(ChainEvent::GlobalParamsUpdated(params));
            }
            ProposalData::SubnetParams { subnet_id, params } => {
                ...
                PalletChain::<T>::deposit_event(ChainEvent::SubnetParamsUpdated(subnet_id));
            }
            ProposalData::TransferDaoTreasury { account, amount } => {
                PalletChain::<T>::transfer_balance_to_account(
...
pub enum ProposalData<T: Config> {
    GlobalCustom,
    GlobalParams(pallet_chain::GlobalParams<T>),
    SubnetCustom {
        subnet_id: SubnetId,
    },
    SubnetParams {
        subnet_id: SubnetId,
        params: pallet_chain::SubnetParams<T>,
    },
    TransferDaoTreasury {
        account: T::AccountId,
        amount: u64,
...
impl<T: Config> Pallet<T> {
    ...
    pub fn add_proposal(
        ...
        let GovernanceConfiguration {
            ...
            pallet_chain::Pallet::<T>::has_enough_balance(&key, proposal_cost),
            Error::<T>::NotEnoughBalanceToPropose
        );

        let Some(removed_balance_as_currency) = PalletChain::<T>::u64_to_balance(proposal_cost)
        else {
            return Err(Error::<T>::InvalidCurrencyConversionValue.into());
        };

        let proposal_id = Self::get_next_proposal_id();
        let current_block = PalletChain::<T>::get_current_block_number();
        ...
        PalletChain::<T>::remove_balance_from_account(&key, removed_balance_as_currency)?;
    ...
    pub fn do_add_transfer_dao_treasury_proposal(
        ...
        let key = ensure_signed(origin)?;
            ...
            pallet_chain::Pallet::<T>::has_enough_balance(
                &DaoTreasuryAddress::<T>::get(),
                value
            ),
    ...
    pub fn do_add_global_params_proposal(
        ...
        pallet_chain::Pallet::check_global_params(&params)?;
...
pub fn get_minimal_stake_to_execute_with_percentage<T: Config>(
    ...
    let stake = match subnet_id {
        Some(specific_subnet_id) => PalletChain::<T>::get_total_subnet_stake(specific_subnet_id),
...
pub fn tick_proposal_rewards<T: Config>(block_number: u64) {
    let mut to_tick: Vec<_> = pallet_chain::N::<T>::iter_keys()
...
fn calc_stake<T: Config>(not_delegating: &BTreeSet<T::AccountId>, voter: &T::AccountId) -> u64 {
    let own_stake = if !not_delegating.contains(voter) {
        ...
        pallet_chain::Pallet::<T>::get_owned_stake(voter)
    };

    let calculate_delegated = || -> u64 {
        PalletChain::<T>::get_stake_from_vector(voter)
            .into_iter()
...
pub fn get_reward_allocation<T: crate::Config>(
    ...
    let treasury_balance = pallet_chain::Pallet::<T>::get_balance(&treasury_address);
    let treasury_balance = I92F36::from_num(pallet_chain::Pallet::<T>::balance_to_u64(
        treasury_balance,
    ));
...
fn distribute_proposal_rewards<T: crate::Config>(
    ...
    for (acc_id, stake) in account_sqrt_stakes.into_iter() {
        ...
        let _ = PalletChain::<T>::transfer_balance_to_account(
