...
pub use pallet::*;
use pallet_chain::{Pallet as ChainMod, SubnetBurn};
use sp_std::vec::Vec;
...
fn submit_dao_application<T: Config>() -> Result<(), &'static str> {
    ...
    ChainMod::<T>::add_balance_to_account(
        &caller,
        ChainMod::<T>::u64_to_balance(1_000_000_000_000_000).unwrap(),
    );
...
benchmarks! {
    ...
    add_global_params_proposal {
        ...
        let params = ChainMod::<T>::global_params();
    ...
    "test".as_bytes().to_vec())?;
        ...
        let params = ChainMod::<T>::subnet_params(netuid);
        let data = "ipfshash".as_bytes().to_vec();

        ChainMod::<T>::update_subnet(
        ...
        ChainMod::<T>::add_balance_to_account(&caller,
        ChainMod::<T>::u64_to_balance(1_000_000_000_000_000).unwrap());
    ...
    add_transfer_dao_treasury_proposal {
        ...
        // Add alice fund to submit the proposal
        ChainMod::<T>::add_balance_to_account(&caller,
    ChainMod::<T>::u64_to_balance(1_000_000_000_000_000).unwrap());     let amount = 1000;
        // Add the amount to treasury funds
        let treasury_address: T::AccountId = DaoTreasuryAddress::<T>::get();
        ChainMod::<T>::add_balance_to_account(&treasury_address,
    ChainMod::<T>::u64_to_balance(amount).unwrap());
    ...
    vote_proposal {
        ...
        // Add Alice's funds to submit the proposal
        ChainMod::<T>::add_balance_to_account(&caller, ChainMod::<T>::u64_to_balance(1_000_000_000_000_000).unwrap());
    ...
    add_dao_application {
        ...
        let application_key: T::AccountId = account("Bob", 0, 2);
        ChainMod::<T>::add_balance_to_account(&caller,
    ChainMod::<T>::u64_to_balance(1_000_000_000_000_000).unwrap());     let data =
