...
use pallet_chain::Pallet as PalletChain;
...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config + pallet_chain::Config {
    ...
    impl<T: Config> ValidateUnsigned for Pallet<T> {
        ...
        fn validate_unsigned(_: TransactionSource, call: &Self::Call) -> TransactionValidity {
            ...
            let key_balance = PalletChain::<T>::get_balance_u64(&key);
            let key_stake: u64 = N::<T>::iter()
                .map(|_| pallet_chain::Pallet::<T>::get_owned_stake(&key))
                .sum();
...
impl<T: Config> Pallet<T> {
    ...
    pub fn do_faucet(
        ...
        let current_block_number: u64 = PalletChain::<T>::get_current_block_number();
        ensure!(
        ...
        let balance_to_add = PalletChain::<T>::u64_to_balance(amount).unwrap();
        PalletChain::<T>::add_balance_to_account(&key, balance_to_add);
    ...
    pub fn hash_block_and_key(
        ...
        // Skip extra 0th byte.
        let key_bytes = binding.get(1..).ok_or(pallet_chain::Error::<T>::ExtrinsicPanicked)?;
        ...
        second_half.copy_from_slice(
            key_bytes.get(..32).ok_or(pallet_chain::Error::<T>::ExtrinsicPanicked)?,
        );
