...
pub struct RootPricing<T: Config> {
    rootnet_id: u16,
    to_be_emitted: u64,
    _pd: PhantomData<T>,
}

impl<T: Config + pallet_chain::Config> RootPricing<T> {
    ...
    pub fn run(self) -> Result<PricedSubnets, sp_runtime::DispatchError> {
        let num_root_validators = pallet_chain::Uids::<T>::iter_prefix(self.rootnet_id).count();
        ...
        let subnet_ids = pallet_chain::N::<T>::iter_keys().collect::<Vec<_>>();
        ...
        for (uid_i, key) in pallet_chain::Keys::<T>::iter_prefix(self.rootnet_id) {
            keys.push((uid_i, key));
        }
        ...
        for ((_, key), stake) in keys.iter().zip(&mut stake_i64) {
            *stake = I64F64::from_num(pallet_chain::Pallet::<T>::get_delegated_stake(key));
        }
        log::debug!("Rootnet# stake {stake_i64:?}.");
        pallet_chain::math::inplace_normalize_64(&mut stake_i64);
        ...
        pallet_chain::math::inplace_row_normalize_64(&mut weights);

        log::debug!("Rootnet# normalized weights {weights:?}.");

        let ranks: Vec<I64F64> = pallet_chain::math::matmul_64(&weights, &stake_i64);
        ...
        for (trust_score, consensus_i) in trust.iter_mut().zip(&mut consensus) {
            let float_kappa = I32F32::from_num(pallet_chain::Kappa::<T>::get())
            ...
            let temperatured_trust = shifted_trust
                .checked_mul(I64F64::from_num(pallet_chain::Rho::<T>::get()))
                .unwrap_or_default();
        ...
        pallet_chain::math::inplace_normalize_64(&mut weighted_emission);
        ...
        let emission_u64: Vec<u64> = pallet_chain::math::vec_fixed64_to_u64(emission_as_com);
    ...
    fn get_root_weights(rootnet_id: u16) -> Vec<Vec<I64F64>> {
        ...
        for (uid_i, weights_i) in Weights::<T>::iter_prefix(rootnet_id) {
