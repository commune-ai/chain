...
use pallet_chain::{Pallet as PalletChain, N};
...
fn process_subnets<T: Config>(block_number: u64, subnets_emission_distribution: PricedSubnets) {
    for netuid in N::<T>::iter_keys() {
        ...
        if pallet_chain::Pallet::<T>::blocks_until_next_epoch(netuid, block_number) == 0 {
            run_epoch::<T>(netuid);
        }
    }
}
...
pub fn run_linear_consensus<T: Config>(
    ...
    let uids = pallet_chain::Keys::<T>::iter_prefix(netuid).collect::<BTreeMap<_, _>>();
...
impl<T: Config> Pallet<T> {
    ...
    pub fn get_lowest_emission_netuid(ignore_subnet_immunity: bool) -> Option<u16> {
        ...
        let immunity_period = pallet_chain::SubnetImmunityPeriod::<T>::get();

        SubnetEmission::<T>::iter()
            .filter(|(netuid, _)| Self::can_remove_subnet(*netuid))
            .filter(|(netuid, _)| pallet_chain::N::<T>::get(netuid) > 0)
            .filter(|(netuid, _)| {
                ignore_subnet_immunity
                    || pallet_chain::SubnetRegistrationBlock::<T>::get(netuid)
                        .map_or(true, |block| {
