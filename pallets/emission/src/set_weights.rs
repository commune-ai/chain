...
use pallet_chain::{Error, Pallet as PalletChain, WeightSettingDelegation};
use sp_core::Get;
...
impl<T: Config> Pallet<T> {
    ...
    fn validate_input_general(
        ...
        netuid: u16,
            ...
            pallet_chain::Pallet::<T>::if_subnet_exist(netuid),
        ...
        Self::validate_uids_length(uids.len(), netuid)?;
            ...
            pallet_chain::Pallet::<T>::is_rootnet(netuid) || !uids.contains(&uid),
    ...
    fn validate_stake(key: &T::AccountId, uids_len: usize) -> DispatchResult {
        let stake = pallet_chain::Pallet::<T>::get_delegated_stake(key);
        let min_stake_per_weight = pallet_chain::MinWeightStake::<T>::get();
    ...
    fn validate_uids_length(len: usize, netuid: u16) -> DispatchResult {
        let min_allowed_length =
            pallet_chain::Pallet::<T>::get_min_allowed_weights(netuid) as usize;
        let max_allowed_length = pallet_chain::MaxAllowedWeights::<T>::get(netuid) as usize;
        ensure!(
    ...
    fn check_whitelisted(netuid: u16, uids: &[u16]) -> DispatchResult {
        ...
        if T::EnforceWhitelist::get() {
            ...
            uids.iter().try_for_each(|&uid| {
                let key = PalletChain::<T>::get_key_for_uid(netuid, uid)
    ...
    fn check_weight_setting_delegation(netuid: u16, key: &T::AccountId) -> DispatchResult {
        if pallet_chain::WeightSettingDelegation::<T>::get(netuid, key).is_some() {
            return Err(pallet_chain::Error::<T>::DelegatingControl.into());
        }
    ...
    pub fn uid_exist_on_network(netuid: u16, uid: u16) -> bool {
        if pallet_chain::Pallet::<T>::is_rootnet(netuid) {
            pallet_chain::N::<T>::contains_key(uid)
        } else {
            pallet_chain::Keys::<T>::contains_key(netuid, uid)
        }
    }
    ...
    fn handle_rate_limiting(uid: u16, netuid: u16, key: &T::AccountId) -> DispatchResult {
        if let Some(max_set_weights) =
            pallet_chain::MaximumSetWeightCallsPerEpoch::<T>::get(netuid).filter(|r| *r > 0)
        {
            let set_weight_uses =
                pallet_chain::SetWeightCallsPerEpoch::<T>::mutate(netuid, key, |value| {
    ...
    fn check_rootnet_daily_limit(netuid: u16, module_id: u16) -> DispatchResult {
        if pallet_chain::Pallet::<T>::is_rootnet(netuid) {
                ...
                pallet_chain::RootNetWeightCalls::<T>::get(module_id).is_none(),
                Error::<T>::MaxSetWeightsPerEpochReached
            );
            pallet_chain::RootNetWeightCalls::<T>::set(module_id, Some(()));
        }
        Ok(())
    }
    ...
    pub fn clear_set_weight_rate_limiter(netuid: u16) {
        let _ = pallet_chain::SetWeightCallsPerEpoch::<T>::clear_prefix(netuid, u32::MAX, None);
    }
    ...
    pub fn do_delegate_weight_control(
        ...
        let Some(target_uid) = pallet_chain::Pallet::<T>::get_uid_for_key(netuid, &target)
        else {
            return Err(Error::<T>::ModuleDoesNotExist.into());
        };

        // Check if target is already delegating their control
        if pallet_chain::WeightSettingDelegation::<T>::get(netuid, &target).is_some() {
    ...
    pub fn do_remove_weight_control(origin: T::RuntimeOrigin, netuid: u16) -> DispatchResult {
        ...
        let Some(_) = pallet_chain::Pallet::<T>::get_uid_for_key(netuid, &key) else {
            return Err(Error::<T>::ModuleDoesNotExist.into());
        };

        if pallet_chain::WeightSettingDelegation::<T>::get(netuid, &key).is_none() {
            return Err(Error::<T>::NotDelegatingControl.into());
        }

        pallet_chain::WeightSettingDelegation::<T>::remove(netuid, &key);
        ...
        F: for<'a> Fn(T::AccountId, u16),
    {
        ...
        {
            let Some(uid) = pallet_chain::Pallet::<T>::get_uid_for_key(netuid, &origin) else {
                continue;
            };
