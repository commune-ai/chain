...
// Chain runtime API
use chain_runtime_api::{ModuleInfo, ModuleParams, ModuleStats};
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
    spec_name: create_runtime_str!("node-chain"),
    impl_name: create_runtime_str!("node-chain"),
    authoring_version: 1,
    spec_version: 515,
    impl_version: 1,
...
parameter_types! {
    ...
    pub const ChainPalletId: PalletId = PalletId(*b"py/subsp");
}
...
impl pallet_chain::Config for Runtime {
    type RuntimeEvent = RuntimeEvent;
    type Currency = Balances;
...
impl_runtime_apis! {
    ...
    impl chain_runtime_api::ChainRuntimeApi<Block> for Runtime {
        fn get_module_info(key: AccountId, netuid: u16) -> ModuleInfo {
            let stats = ChainModule::get_module_stats(netuid, &key);
            let uid = ChainModule::get_uid_for_key(netuid, &key).unwrap_or_default();
            let params = ChainModule::module_params(netuid, &key, uid);
